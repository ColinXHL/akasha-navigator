...
public class HotkeyConfig
{
    ...
    public static HotkeyConfig CreateDefault()
    {
        var config =
            new HotkeyConfig {
                ActiveProfileName = "Default", AutoSwitchProfile = false,
                Profiles =
                    new List<HotkeyProfile> {
                        new HotkeyProfile {
                            Name = "Default", ActivationProcesses = null,
                            Bindings =
                                new List<HotkeyBinding> {// 5 键 - 视频倒退
                                                         new HotkeyBinding { Key = 0x35, Modifiers = ModifierKeys.None,
                                                                             Action = "SeekBackward" },
                                                         // 6 键 - 视频前进
                                                         new HotkeyBinding { Key = 0x36, Modifiers = ModifierKeys.None,
                                                                             Action = "SeekForward" },
                                                         // ` 波浪键 - 播放/暂停
                                                         new HotkeyBinding { Key = 0xC0, Modifiers = ModifierKeys.None,
                                                                             Action = "TogglePlay" },
                                                         // 7 键 - 降低透明度
                                                         new HotkeyBinding { Key = 0x37, Modifiers = ModifierKeys.None,
