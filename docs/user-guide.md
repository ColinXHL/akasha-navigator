...
1. 打开「插件中心」（菜单 → 插件中心）
2. 切换到「可用插件」选项卡
3. 根据需要更新插件仓库，点击插件的「安装」按钮

插件中心展示的是
[AkashaPlugins](https://github.com/ColinXHL/akasha-plugins) catalog。
从这里安装会同时建立订阅；catalog 是插件版本、下载包和更新信息的唯一来源。

### 检查和安装更新

「已安装插件」页的检查更新、应用启动时的更新提示以及插件中心使用同一份订阅
和 catalog 数据，因此结果一致。可以在插件仓库设置中分别控制：

- 自动更新插件仓库：只刷新 catalog，不替换本地插件
- 自动更新已订阅插件：安装 catalog 中更高版本的订阅插件

应用自身的版本更新与插件更新相互独立；应用更新清单不包含插件版本。
...
在「可用插件」页选择「从 ZIP 安装」可导入本地插件包。手动导入不会把任意
ZIP 当作官方订阅；需要后续自动更新时，请从 catalog 中安装或订阅对应插件。
...
开荒笔记功能允许你保存和组织游戏攻略相关的网页内容，使用树形结构管理，支持文件夹分类。
...
- **编辑**：修改标题和 URL
- **移动到...**：将笔记项移动到其他文件夹
- **删除**：删除笔记项或文件夹（删除文件夹会同时删除其中所有内容）
- **新建子目录**：在文件夹下创建子文件夹

### 数据存储

开荒笔记数据存储在当前 Profile 的 `pioneer-notes.json` 文件中，切换 Profile 会显示对应的笔记内容。
